...
function isValidEIN(ein) {
  return /^([07][1-7]|1[0-6]|2[0-7]|[35][0-9]|[468][0-8]|9[0-589])-?\d{7}$/.test(
    ein
  );
}
...
$(document).ready(function () {
  ...
  $("#business-contact").submit(function (e) {
    e.preventDefault();
    submitBusinessContactDetailsForm(e, this);
  });

  function submitBusinessContactDetailsForm(e, formElement) {
    e.preventDefault();
    var loggedInUser = getLoggedInPartner();
    if (!loggedInUser) {
      alert("Your session has expired, please login to continue");
    ...
    var userId = loggedInUser.partner_id;
    if (!userId) {
    ...
    var error = [];
    if (!data.fname) error.push("<li>First Name is required</l1>");
    if (!data.lname) error.push("<li>Last Name is required</l1>");
    if (!data.contact) error.push("<li>Contact Number is required</l1>");

    $(".page-loader-wrapper").show();
    updateBusinessContact(userId, data);
  }

  $("#business-password").submit(function (e) {
    e.preventDefault();
    ChangedPassowrdForm(e, this);
  });

  function ChangedPassowrdForm(e, formElement) {
    e.preventDefault();
    var loggedInUser = getLoggedInPartner();
    if (!loggedInUser) {
      alert("Your session has expired, please login to continue");
    ...
    var userId = loggedInUser.auth_id;
    if (!userId) {
      return showErrorNotification();
    }
    ...
    var error = [];
    if (!data.old_password) error.push("<li>Old Password is required</l1>");
    if (!data.new_password) error.push("<li>New Password is required</l1>");
    if (!data.confirm_password)
      error.push("<li>Confirm Password is required</l1>");
  ...
  $("#business-info").submit(function (e) {
    e.preventDefault();
    submitBusinessInfoDetailsForm(e, this);
  });

  function submitBusinessInfoDetailsForm(e, formElement) {
    e.preventDefault();
    var loggedInUser = getLoggedInPartner();
    ...
    var error = [];
    if (!data.streetAddress) error.push("<li>Street Address is required</l1>");
    if (!data.postal) error.push("<li>Postal Code is required</l1>");
    if (!data.city) error.push("<li>City is required</l1>");
    if (!data.state) error.push("<li>State is required</l1>");
    // console.log(data);
    $(".page-loader-wrapper").show();
    updateBusinessInfo(userId, data);
  }
  ...
  $(".other-owner-details-row:first")
    .find(".other-owner-state")
  ...
  $("#business-credit-application-form .submit-button").on(
    "click",
    function (e) {
      e.preventDefault();
      $("#business-credit-application-form").trigger("submit");
  ...
  $("#btn-clone-other-owner").on("click", function (e) {
    ...
    clonnedRow
      .find(".other-owner-first-name")
      .attr("name", "other-owner-first-name[]")
      .val("");
    clonnedRow
      .find(".other-owner-last-name")
      .attr("name", "other-owner-last-name[]")
      .val("");
    clonnedRow
      .find(".other-owner-ssn")
      .attr("name", "other-owner-ssn[]")
      .val("");
    clonnedRow
      .find(".other-owner-ownership")
      .attr("name", "other-owner-ownership[]")
      .val("");
    clonnedRow
      .find(".other-owner-address")
      .attr("name", "other-owner-address[]")
      .val("");
    clonnedRow
      .find(".other-owner-city")
      .attr("name", "other-owner-city[]")
      .val("");
    clonnedRow
      .find(".other-owner-zip")
      .attr("name", "other-owner-zip[]")
      .val("");
  ...
  function submitCapitalApplicationForm(e, formElement) {
    ...
    var data = {
      ...
      phone_number: $("#phone").val(),
      email_address: $("#email").val(),
      owner_date_of_birth: !!$("#dob").val()
        ? moment($("#dob").val()).format("MM/DD/YYYY")
        : "",
      owner_home_address: $("#home-address").val(),
      owner_city: $("#city").val(),
      owner_state: $("#state").val(),
  ...
  $("#federal-tax-id, .percentage-input, .number-input").on(
    "keypress",
    function (e) {
      const key = e.which || e.keyCode;
      if (!(key >= 48 && key <= 57)) {
  ...
  $("#cb-same-as-company-details").on("change", function (e) {
    ...
    if ($(this).is(":checked")) {
      ...
      $("#dba-street-address")
        .val($("#company-street-address").val())
        .prop("disabled", true);
      $("#dba-city").val($("#company-city").val()).prop("disabled", true);
      $("#dba-state")
        .val($("#company-state").val())
  ...
  function submitCompanyAndDbaDetailsForm(e, formElement) {
    ...
    var data = {
      ...
      founded_date: $("#founded-date").val()
        ? moment($("#founded-date").val()).format("MM/DD/YYYY")
        : "",
    ...
    if (!data.company_street_address)
      error.push("<li>Company Street Address</l1>");
  ...
  function submitMerchantBusinessDetailsForm(e, formElement) {
    ...
    if (!data.customer_service_phone)
      error.push("<li>Customer Service Phone Number</l1>");
    if (!data.has_bankruptcy) error.push("<li>Has Bankruptcy</l1>");
    if (data.has_bankruptcy && data.has_bankruptcy !== "Never") {
      if (!data.bankruptcy_description)
        error.push("<li>Bankruptcy Description</l1>");
    }
    ...
    if (!isValidUSPhoneNumber(data.customer_service_phone))
      error.push("<li>Customer service phone number is not valid</l1>");
    if (
      !!data.customer_service_email &&
      !isValidEmail(data.customer_service_email)
    )
      error.push("<li>Customer service email is not valid</l1>");
    if (!!data.corporate_phone && !isValidUSPhoneNumber(data.corporate_phone))
      error.push("<li>Corporate phone number is not valid</l1>");
    if (!!data.corporate_email && !isValidEmail(data.corporate_email))
      error.push("<li>Corporate email is not valid</l1>");
    if (!!data.corporate_fax && !isValidUSPhoneNumber(data.corporate_fax))
      error.push("<li>Corporate fax number is not valid</l1>");
  ...
  function submitMerchantPricipalForm(e, formElement) {
    ...
    var data = {
      ...
      dob: $("#principal-dob").val()
        ? moment($("#principal-dob").val()).format("MM/DD/YYYY")
        : "",
      license_number: $("#driver-license-number").val(),
      license_state: $("#driver-license-state").val(),
      license_expiration: $("#driver-license-expiration").val()
        ? moment($("#license-expiration").val()).format("MM/DD/YYYY")
        : "",
    ...
    if (!data.license_expiration)
      error.push("<li>Driver's License Expiration</l1>");
    ...
    if (!isValidUSPhoneNumber(data.phone_number))
      error.push("<li>Phone number is not valid</l1>");
    if (!isValidEmail(data.email_address))
      error.push("<li>Email address is not valid</l1>");
  ...
  function submitMerchantProcessingForm(e, formElement) {
    ...
    var data = {
      ...
      fulfillment_delivery_other: $(
        "#fulfillment-policy-deliverly-other"
      ).val(),
    ...
    if (!data.monthly_transaction_amount)
      error.push("<li>Monthly Transaction Amount</l1>");
    if (!data.average_transaction_amount)
      error.push("<li>Average Transaction Amount</l1>");
    if (!data.max_transaction_amount)
      error.push("<li>Max Transaction Amount</l1>");
    if (!data.sales_swiped_percentage)
      error.push("<li>Sales Swiped Percentage</l1>");
    if (!data.sales_mail_percentage)
      error.push("<li>Sales Mail Percentage</l1>");
    if (!data.sales_internet_percentage)
      error.push("<li>Sales Internet Percentage</l1>");
    if (!data.is_already_processing)
      error.push("<li>Is Already Processing</l1>");
    ...
    if (!data.intended_usage_credit_cards)
      error.push("<li>Intended Usage Credit Card</l1>");
    if (!data.intended_usage_pin_debit)
      error.push("<li>Intended Usage Pin Debit</l1>");
    if (!data.intended_usage_ebt) error.push("<li>Intended Usage EBT</l1>");
    if (!data.has_onsite_inventory)
      error.push("<li>Has On-Site Inventory?</l1>");
    if (!data.has_offsite_inventory)
      error.push("<li>Has Off-Site Inventory?</l1>");
    if (data.has_offsite_inventory === "Yes") {
      if (!data.offsite_inventory_address)
        error.push("<li>Off-Site Inventory Address</l1>");
    }
    if (!data.has_3rd_party_inventory)
      error.push("<li>Using 3rd party fulfillment center?</l1>");
    if (!data.has_service_only_inventory)
      error.push("<li>Service only business?</l1>");
    ...
    let salesPercentage =
      data.sales_swiped_percentage +
      data.sales_mail_percentage +
      data.sales_internet_percentage;
    if (salesPercentage != 100) {
      error.push(
        "<li>Sales (swiped + mail + internet) percentage is not equal to 100</l1>"
      );
    }

    let customersPercentage =
      data.individual_customers_percentage +
      data.business_customers_percentage +
      data.government_customers_percentage;
    if (customersPercentage != 100) {
      error.push(
        "<li>Individual + Business + Government customers percentage is not equal to 100</l1>"
      );
    }

    let customerSalesPercentage =
      data.local_sales_percentage +
      data.national_sales_percentage +
      data.international_sales_percentage;
    if (customerSalesPercentage != 100) {
      error.push(
        "<li>Local + National + International sales percentage is not equal to 100</l1>"
      );
    }
  ...
  function submitMerchantACHForm(e, formElement) {
    ...
    var data = {
      ...
      individual_customers_percentage: $(
        "#ach-individual-customers-percentage"
      ).val(),
      business_customers_percentage: $(
        "#ach-business-customers-percentage"
      ).val(),
      government_customers_percentage: $(
        "#ach-government-customers-percentage"
      ).val(),
      local_sales_percentage: $("#ach-local-sales-percentage").val(),
      national_sales_percentage: $("#ach-national-sales-percentage").val(),
      international_sales_percentage: $(
        "#ach-international-sales-percentage"
      ).val(),
      ...
      recurring_payments_description: $(
        "#ach-recurring-payments-description"
      ).val(),
    ...
    if (!data.max_transaction_amount)
      error.push("<li>Max Transaction Amount</l1>");
    if (!data.has_issue_credit)
      error.push("<li>Will You Need To Issue Credit?</l1>");
    ...
    if (data.fulfillment_delivery === "other") {
      if (!data.fulfillment_delivery_other)
        error.push("<li>Other Fulfillment Delivery</l1>");
    }
  ...
  $(
    "#phone,#processing-volume,#business-phone,#business-zip,#admin-phone,#technical-phone,#mailing-address-zip"
  ...
  $(
    "#merchant-application-submit-button,#merchant-application-form button[type=submit]"
  ).on("click", function (e) {
    e.preventDefault();
  ...
  function submitMerchantApplicationForm(e, formElement) {
    ...
    if (!data.mailing_address_state)
      error.push("<li>Mailing Address State</l1>");
    if (!data.mailing_address_country)
      error.push("<li>Mailing Address Country</l1>");
...
function loadUserDetailsInForm(user) {
  ...
  $("#founded-date").val(
    user.business_start_date
      ? moment(user.business_start_date).format("DD/MM/YYYY")
      : ""
  );
...
function updateBusinessInfo(userId, data) {
  ...
  $.ajax({
    type: "POST",
    url: `${apiBaseURL}/updateBusinessInfo`,
    contentType: "application/json",
    data: JSON.stringify(data),
    success: function (response) {
      console.log(response);
      ...
      if (data && response.status == 1) {
        ...
        showSuccessNotification("Partner details updated successfully");
        location.reload();
      } else {
        showErrorNotification(response.message);
      }
...
function updateBusinessContact(userId, data) {
  data["partner_id"] = userId;
  var loggedInUser = getLoggedInPartner();

  $.ajax({
    type: "POST",
    url: `${apiBaseURL}/updateBusinessContact`,
    contentType: "application/json",
    data: JSON.stringify(data),
    success: function (response) {
      $(".page-loader-wrapper").hide();
...
function loadCreditReportsInTable(data) {
  ...
  $.each(data, function (key, value) {
    ...
    user += "<td>" + value.first_name + " " + value.last_name + "</td>";

    user += "<td>" + value.phone + "</td>";

    user += "<td>" + value.email + "</td>";

    user += "<td>" + value.dob + "</td>";

    user += "<td>" + value.ssn + "</td>";

    user +=
      '<td><div class="flex align-items-center list-user-action">' +
      '<a class="iq-bg-primary view" data-toggle="tooltip" data-placement="top" id="view' +
      value.ID +
      '" title="View User" data-original-title="View" href="javscript:void(0)"><i class="ri-eye-line"></i></a>' +
      '<a class="iq-bg-primary edit" data-toggle="tooltip" data-placement="top" id="edit' +
      value.ID +
      '" title="Edit User" data-original-title="Edit" href="#"><i class="ri-pencil-line"></i></a>' +
      '<a class="iq-bg-primary delete" data-toggle="tooltip" data-placement="top" id="delete' +
      value.ID +
      '" title="Delete User" data-original-title="Delete" href="#"><i class="ri-delete-bin-line"></i></a>';
    if (value.status == 0) {
      user +=
        '<a class="iq-bg-primary" data-toggle="tooltip" data-placement="top" id="active' +
        value.ID +
        '" title="Active User" data-original-title="Delete" href="#"><i class="ri-checkbox-circle-fill" style="color:green"></i></i></a>';
    } else {
      user +=
        '<a class="iq-bg-primary" data-toggle="tooltip" data-placement="top" id="' +
        value.ID +
        '" title="Deactive User" data-original-title="Delete" href="#"><i class="ri-indeterminate-circle-fill" style="color:red"></i></i></a>' +
        "</div></td>";
    }
    user += "</tr>";
  });
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (company) {
        ...
        $("#company-street-address").val(
          company.address ? company.address.street : ""
        );
        $("#company-city").val(company.address ? company.address.city : "");
        $("#company-state").val(
          company.address ? company.address.state.code : ""
        );
        $("#company-zip").val(company.address ? company.address.zip : "");
        $("#founded-date").val(
          company.founded ? moment(company.founded).format("DD/MM/YYYY") : ""
        );
      }
      if (dba) {
        $("#cb-same-as-company-details").prop(
          "checked",
          dba.sameAsComapny === "Yes"
        );
      ...
      if (application.businessLocation) {
        ...
        $("#business-locations-count").val(
          application.businessLocation.numberOfLocations
        );
        $("#building-ownership").val(
          application.businessLocation.buildingOwnership
        );
      ...
      if (application.customerServiceContact) {
        $("#customer-service-phone").val(
          application.customerServiceContact.phone
        );
        $("#customer-service-email").val(
          application.customerServiceContact.email
        );
      }
      ...
      if (processing) {
        ...
        if (processing.volumes) {
          $("#monthly-transaction-amount").val(
            processing.volumes.monthlyTransactionAmount
          );
          $("#average-transaction-amount").val(
            processing.volumes.avgTransactionAmount
          );
          $("#max-transaction-amount").val(
            processing.volumes.maxTransactionAmount
          );
        }
        if (processing.sales) {
        ...
        if (processing.alreadyProcessing) {
          $("#is-already-processing").val(
            processing.alreadyProcessing.isProcessing
          );
        ...
        if (processing.customers) {
          if (processing.customers.type) {
            $("#individual-customers-percentage").val(
              processing.customers.type.individual
            );
            $("#business-customers-percentage").val(
              processing.customers.type.business
            );
            $("#government-customers-percentage").val(
              processing.customers.type.government
            );
          }
          if (processing.customers.location) {
            $("#local-sales-percentage").val(
              processing.customers.location.local
            );
            $("#national-sales-percentage").val(
              processing.customers.location.national
            );
            $("#international-sales-percentage").val(
              processing.customers.location.international
            );
          }
        }
        if (processing.fulfillmentPolicy) {
          $("#fulfillment-policy-fulfillment").val(
            processing.fulfillmentPolicy.fulfillment
          );
          $("#fulfillment-policy-fulfillment-other").val(
            processing.fulfillmentPolicy.fulfillmentOther
          );
          $("#fulfillment-policy-deliverly").val(
            processing.fulfillmentPolicy.delivery
          );
          $("#fulfillment-policy-deliverly-other").val(
            processing.fulfillmentPolicy.deliveryOther
          );
        }
        if (processing.recurringPayments) {
          $("#has-recurring-payments").val(
            processing.recurringPayments.hasRecurring
          );
          $("#recurring-payments-description").val(
            processing.recurringPayments.description
          );
        }
        if (processing.intendedUsage) {
          $("#intended-usage-credit-cards").val(
            processing.intendedUsage.creditCards
          );
          $("#intended-usage-pin-debits").val(
            processing.intendedUsage.pinDebit
          );
        ...
        if (processing.seasonalBusiness) {
          $("#is-business-seasonal").val(
            processing.seasonalBusiness.isSeasonal
          );
          if (processing.seasonalBusiness.months) {
            processing.seasonalBusiness.months.forEach((month) => {
              $(`.business-months input[value|='${month}']`).prop(
                "checked",
                true
              );
            });
        ...
        if (processing.inventory) {
          ...
          $("#offsite-inventory-address").val(
            processing.inventory.offSiteAddress
          );
          $("#has-3rd-party-fulfillment-center").val(
            processing.inventory.thirdParty
          );
      ...
      if (ach) {
        ...
        if (ach.volumes) {
          ...
          $("#ach-monthly-transaction-amount").val(
            ach.volumes.monthlyTransactionAmount
          );
          $("#ach-average-transaction-amount").val(
            ach.volumes.avgTransactionAmount
          );
          $("#ach-max-transaction-amount").val(
            ach.volumes.maxTransactionAmount
          );
        }
        if (ach.issueCredits) {
          $("#ach-has-issue-credit").val(
            ach.issueCredits.hasIssueCredits
              ? ach.issueCredits.hasIssueCredits
              : "No"
          );
        ...
        if (ach.alreadyProcessing) {
          $("#ach-is-processing").val(
            ach.alreadyProcessing.isProcessing
              ? ach.alreadyProcessing.isProcessing
              : "No"
          );
        ...
        if (ach.terminated) {
          $("#ach-is-terminated").val(
            ach.terminated.isTerminated ? ach.terminated.isTerminated : "No"
          );
          $("#ach-termination-reason").val(ach.terminated.description);
        }
        if (ach.customers) {
          if (ach.customers.type) {
            $("#ach-individual-customers-percentage").val(
              ach.customers.type.individual
            );
            $("#ach-business-customers-percentage").val(
              ach.customers.type.business
            );
            $("#ach-government-customers-percentage").val(
              ach.customers.type.government
            );
          }
          if (ach.customers.location) {
            $("#ach-local-sales-percentage").val(ach.customers.location.local);
            $("#ach-national-sales-percentage").val(
              ach.customers.location.national
            );
            $("#ach-international-sales-percentage").val(
              ach.customers.location.international
            );
          }
        }
        if (ach.fulfillmentPolicy) {
          $("#ach-fulfillment-deliverly").val(ach.fulfillmentPolicy.delivery);
          $("#ach-fulfillment-deliverly-other").val(
            ach.fulfillmentPolicy.deliveryOther
          );
        }
        if (ach.recurringPayments) {
          $("#ach-has-recurring-payments").val(
            ach.recurringPayments.hasRecurring
          );
          $("#ach-recurring-payments-description").val(
            ach.recurringPayments.description
          );
        }
...
function loadCardInTable(data) {
  ...
  $.each(data, function (key, value) {
    ...
    user += "<td>" + value.company_name + "</td>";

    user += "<td>" + value.administrator_email_address + "</td>";

    user += "<td>" + value.administrator_phone_number + "</td>";

    user += "<td>" + value.tax_id + "</td>";

    user += "<td>" + value.parent_company_name + "</td>";
...
function loadCreditSafeInTable(data) {
  ...
  $.each(data, function (key, value) {
    if (value.first_name) {
      ...
      user += "<td>" + value.company_id + "</td>";

      user +=
        "<td>" +
        JSON.parse(value.searchData).companySummary.businessName +
        "</td>";

      user += "<td>" + value.searched_at + "</td>";
...
function loadBusinessCreditScoresInTable(data) {
  ...
  $.each(data, function (key, value) {
    if (value.dnb != null || value.experian != null) {
      ...
      bscore += "<td>" + value.first_name + " " + value.last_name + "</td>";

      bscore += "<td>" + value.application_id + "</td>";

      bscore += "<td>" + value.dnb + "</td>";

      bscore += "<td>" + value.experian + "</td>";

      bscore += "<td>" + value.created_at + "</td>";
...
function loadPersonalCreditScoresInTable(data) {
  ...
  $.each(data, function (key, value) {
    if (value.experian != 0 || value.equifax != 0 || value.transunion != 0) {
      ...
      bscore += "<td>" + value.equifax + "</td>";

      bscore += "<td>" + value.transunion + "</td>";
...
// let codeIDOne = document.getElementById("embedded-code-one");
// let codeIDtwo = document.getElementById("embedded-code-two");
// let one =
//   "<script src='https://admin.kaped.io/assets/custom/embedded/script.js'>";
// let two = "</";
// let three = "script>";
// let four = "<iframe id='kaped-partner-";
// let five = "frame'></iframe>";
// codeIDOne.innerText = `${one}${two}${three}`;
// codeIDtwo.innerText = `${four}${five}`;

let affiliateCodeIDOne = document.getElementById("embedded-code-affiliate-one");
let affiliateCodeIDtwo = document.getElementById("embedded-code-affiliate-two");
let affiliateOne =
  "<script src='https://www.artechnity.in/kaped/assets/custom/embedded/script-credit-card-application.js'>";
let affiliateTwo = "</";
let affiliateThree = "script>";
let affiliateFour = `<iframe id='partner-public-kaped-iframe' data-value='${PartnerID}'`;
let affiliateFive = "></iframe>";
affiliateCodeIDOne.innerText = `${affiliateOne}${affiliateTwo}${affiliateThree}`;
affiliateCodeIDtwo.innerText = `${affiliateFour}${affiliateFive}`;
